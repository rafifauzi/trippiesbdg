...
if (isset($_POST['pasarkan'])) {
    ...
    $inputBarang=mysql_query("INSERT INTO `tb_barang` (`id_barang`, `nama_barang`, `stok`, `berat`, `harga_beli`, `harga_jual`, `kondisi`, `dilihat`, `deskripsi`, `tgl_upload`, `id_kategori`, `pemilik`) VALUES ('$idBarang', '$nmBarang', '$qty', '$brtBarang', '$hgBeli', '$hgJual', '$kondisi', '0', '$deskripsi', '$date', '$kBarang', '$idUser');");
...
if (isset($_POST['hapusBarang'])) {
    ...
    if ($count>0 && $hapusBarang && $hapusDetailBarang) {
        ?>
        <script type="text/javascript">
            alertify.alert("<?=$count;?> Barang Telah Dihapus", function(){ window.location.assign('list-barang'); }).setHeader(' ').set({closable:false,transition:'pulse'});
        </script>
        <?php
    }else{
        ?>
